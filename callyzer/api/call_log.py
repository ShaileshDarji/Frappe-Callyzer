...
def fetch_never_attended_calls():
	...
	for setting in settings:
		...
		while True:
			...
			try:
				result = post_api(url, token, payload, page_no=page_no)
			except Exception:
			...
			if result is None:
				frappe.log_error(
					f"Callyzer: 400 on page {page_no} | call_from: {call_from} | call_to: {call_to} | emp_count: {len(employee_ids)} | emp_sample: {employee_ids[:5]}",
					"Callyzer Pagination 400 - Never Attended"
				)
				break
...
def fetch_not_pickup_by_client_calls():
	...
	for setting in settings:
		...
		while True:
			...
			if result is None:
				frappe.log_error(
					f"Callyzer: 400 on page {page_no} | call_from: {call_from} | call_to: {call_to} | emp_count: {len(employee_ids)} | emp_sample: {employee_ids[:5]}",
					"Callyzer Pagination 400 - Not Pickup By Client"
				)
				break
	...
	update_last_fetched_time(end_point_name)
	return {"status": "success", "message": "Not pickup by client calls fetched successfully"}
...
def process_unique_clients_response(result, company):
	...
	for client in clients:
		...
		doc.total_incoming_duration = client.get("total_incoming_duration")
		doc.total_outgoing_duration = client.get("total_outgoing_duration")
		doc.total_missed_calls = client.get("total_missed_calls")
		doc.total_rejected_calls = client.get("total_rejected_calls")
		doc.total_connected_calls = client.get("total_connected_calls")
		doc.total_never_attended = client.get("total_never_attended")
		doc.total_not_picked_by_client = client.get("total_not_picked_by_client")
		doc.last_call_details = json.dumps(client.get("last_call_log"))
		doc.company = company

		last_call_log = client.get("last_call_log", {})
		if last_call_log:
			last_call_date = last_call_log.get("call_date")
			doc.last_call_date = getdate(last_call_date)
...
def fetch_day_wise_analytics_report(call_from=None, call_to=None):
	...
	if call_from and call_to:
		...
		endpoint = get_endpoint(end_point_name)[0]
	else:
		endpoint, call_from, call_to = get_endpoint(end_point_name)
...
def fetch_call_history_report(call_from=None, call_to=None):
	...
	if call_from and call_to:
		...
		call_to = calendar.timegm(datetime.combine(call_to_date, time(23, 59, 59)).timetuple())
		endpoint = get_endpoint(end_point_name)[0]
	else:
		endpoint, call_from, call_to = get_endpoint(end_point_name)
	...
	for setting in settings:
		...
		while True:
			...
			if result is None:
				frappe.log_error(
					f"Callyzer: 400 on page {page_no} | call_from: {call_from} | call_to: {call_to} | emp_count: {len(employee_ids)} | emp_sample: {employee_ids[:5]}",
					"Callyzer Pagination 400 - Call History"
				)
				break
...
def process_call_history_response(result, company):
	...
	for call in call_logs:
		...
		doc.employee = call.get("emp_number")
		doc.employee_name = call.get("emp_name")
		doc.employee_code = call.get("emp_code")
		doc.employee_number = call.get("emp_number")
		doc.employee_country_code = call.get("emp_country_code")
		doc.employee_tags = ", ".join(call.get("emp_tags") or [])
...
def fetch_call_history_report_daily():
	...
	today_date = getdate()
	call_from_dt = datetime.combine(today_date, time.min)
	call_to_dt = datetime.combine(today_date, time.max)
	...
	for setting in settings:
		...
		while True:
			...
			try:
				...
				frappe.log_error("Callyzer: Call History Daily page fetch failed", frappe.get_traceback())
				break

			if result is None:
				frappe.log_error(
					f"Callyzer: 400 on page {page_no} | call_from: {call_from} | call_to: {call_to} | emp_count: {len(employee_ids)} | emp_sample: {employee_ids[:5]}",
					"Callyzer Pagination 400 - Call History Daily"
				)
				break
...
@frappe.whitelist()
def remove_call_recording(unique_ids: list, company: str = None):
	if not unique_ids:
...
def get_api(url, api_key, payload):
	...
	for attempt in range(max_retries):
		try:
			try:
				...
				frappe.log_error("Callyzer GET API timed out (30s). Retrying with 60s timeout.", "Callyzer Timeout Retry")
				response = requests.get(url, headers=headers, data=json.dumps(payload), timeout=60)

			if response.status_code == 429:
				wait = 2 ** attempt
				frappe.log_error(
					f"Callyzer GET: Rate limited (429) on attempt {attempt + 1}/{max_retries}. Waiting {wait}s.",
					"Callyzer Rate Limit"
				)
				time_module.sleep(wait)
				continue
	...
	frappe.log_error(
		f"Callyzer GET API rate-limited after {max_retries} retries. Skipping request to {url}.",
		"Callyzer Rate Limit Exceeded"
	)
	return {}
